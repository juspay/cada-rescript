...
def get_changed_files(branch_name, new_commit, local_path):
    old_cwd = os.getcwd()
    os.chdir(local_path)

    try:
        subprocess.run(['git', 'checkout', '-f', branch_name], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        commit = subprocess.run(['git', 'rev-parse', branch_name], check=True, stdout=subprocess.PIPE).stdout.decode().strip()
...
def main():
    ...
    if len(args) != 5:
        print('Usage: python rescript_differ.py <repo_url> <local_path> <branch_name> <current_commit> <output_dir>')
        sys.exit(1)

    repo_url, local_repo_path, branch_name, current_commit, output_dir = args
    ...
    changed_files = get_changed_files(branch_name, current_commit, local_repo_path)
    ...
    for module_name, asts_list in file_node_dict.items():
        if len(asts_list) == 2:
            ...
            diff = RescriptFileDiff(old_node, new_node)
